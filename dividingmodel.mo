...
equation
  ...
  Bat1Load = if total > 0 and SOC1 > 0.001 and SOC2 > 0.001 and Load > 0 then Load * (RUL1 / total) 
    ...
    elseif SOC1 > 0.001 and SOC2 < 0.001 and Load > 0 and RUL1 > 0.001 then Load    
    elseif SOC1 < .999 and SOC2 > .999 and Load < 0 and RUL1 > 0.001 then Load
    else 0;
  Bat2Load = if total > 0 and SOC2 > 0.001 and SOC1 > 0.001 and Load > 0 then Load * (RUL2 / total)   //If RUL is equal share load
    ...
    //Send full load to battery if one is full or empty
    elseif SOC2 > 0.001 and SOC1 < 0.001 and Load > 0 and RUL2 > 0.001 then Load 
    elseif SOC2 < .999 and SOC1 > .999 and Load < 0 and RUL2 > 0.001 then Load
    else 0;
end dividingmodel;
